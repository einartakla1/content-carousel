...
(function () {
    ...
    // Helper function to get the appropriate link URL for a slide
    // Priority: Individual link > Global link
    function getSlideLink(slide) {
        // Individual link takes priority
    ...
    // Helper function to wrap URL with GAM tracking if available
    function wrapWithGamTracking(url, logPrefix) {
        if (!url) return url;

        var finalUrl = url;
        console.log(`[Carousel] ${logPrefix} - Original URL:`, url);
        console.log(`[Carousel] ${logPrefix} - window.gamClickUrl:`, window.gamClickUrl);
        ...
        if (isValidGamUrl) {
            finalUrl = window.gamClickUrl + encodeURIComponent(url);
            console.log(`[Carousel] ${logPrefix} - Wrapped URL:`, finalUrl);
        } else {
            console.log(`[Carousel] ${logPrefix} - Using original URL (no valid GAM tracking URL)`);
        }

        return finalUrl;
    }

    // Helper function to add click handler to a slide box
    function addSlideClickHandler(box, slide, slideIndex, hasButton) {
        // Don't add click handler if slide has a button (button handles its own clicks)
        if (hasButton) {
            return;
        }
        ...
        if (linkUrl) {
            ...
            box.addEventListener('click', () => {
                // Track slide click if analytics is available
                if (globalTrackEvent) {
                    globalTrackEvent('slide_click', {
                ...
                const finalUrl = wrapWithGamTracking(linkUrl, 'Slide Click');
    ...
    function injectStyles() {
        ...
        style.textContent = `
            ...
            @media (max-width: 600px) {
                ...
                /* Apply mobile font sizes using CSS custom properties */
                .text-overlay-title {
                    font-size: var(--title-font-size-mobile) !important;
                }

                .text-overlay-content {
                    font-size: var(--text-font-size-mobile) !important;
                }
                ...
                .big-number-box-number {
                    font-size: var(--number-font-size-mobile) !important;
                }
            ...
            /* Very small screens - responsive slide width */
    ...
    function renderSlide(slide, slideIndex) {
        switch (slide.type) {
            case 'fullImage':
                return renderFullImage(slide, slideIndex);
            case 'fullImageWithText':
                return renderFullImageWithText(slide, slideIndex);
            case 'text':
                return renderText(slide, slideIndex);
            case 'video':
                return renderVideo(slide, slideIndex);
            case 'fullVideoWithText':
                return renderFullVideoWithText(slide, slideIndex);
            case 'bigNumber':
                return renderBigNumber(slide, slideIndex);
            case 'imageSelector':
                return renderImageSelector(slide, slideIndex);
            default:
    ...
    function createTextOverlay(slide) {
        ...
        if (slide.title) {
            ...
            // Apply font sizes with CSS custom properties for responsive design
            const desktopFontSize = slide.titleFontSize || 24;
            const mobileFontSize = slide.titleFontSizeMobile || slide.titleFontSize || 16;
            title.style.setProperty('--title-font-size-desktop', desktopFontSize + 'px');
            title.style.setProperty('--title-font-size-mobile', mobileFontSize + 'px');
            title.style.fontSize = 'var(--title-font-size-desktop)';
        ...
        if (slide.text) {
            ...
            // Apply font sizes with CSS custom properties for responsive design
            const desktopFontSize = slide.textFontSize || 14;
            const mobileFontSize = slide.textFontSizeMobile || slide.textFontSize || 12;
            text.style.setProperty('--text-font-size-desktop', desktopFontSize + 'px');
            text.style.setProperty('--text-font-size-mobile', mobileFontSize + 'px');
            text.style.fontSize = 'var(--text-font-size-desktop)';
    ...
    function renderText(slide) {
        ...
        // Apply font sizes with CSS custom properties for responsive design
        const titleDesktopSize = slide.titleFontSize || 22;
        const titleMobileSize = slide.titleFontSizeMobile || slide.titleFontSize || 18;
        title.style.setProperty('--title-font-size-desktop', titleDesktopSize + 'px');
        title.style.setProperty('--title-font-size-mobile', titleMobileSize + 'px');
        title.style.fontSize = 'var(--title-font-size-desktop)';
        ...
        // Apply font sizes with CSS custom properties for responsive design
        const textDesktopSize = slide.textFontSize || 16;
        const textMobileSize = slide.textFontSizeMobile || slide.textFontSize || 14;
        text.style.setProperty('--text-font-size-desktop', textDesktopSize + 'px');
        text.style.setProperty('--text-font-size-mobile', textMobileSize + 'px');
        text.style.fontSize = 'var(--text-font-size-desktop)';
    ...
    function renderBigNumber(slide) {
        ...
        // Apply font sizes with CSS custom properties for responsive design
        const numberDesktopSize = slide.numberFontSize || 40;
        const numberMobileSize = slide.numberFontSizeMobile || slide.numberFontSize || 32;
        numberOne.style.setProperty('--number-font-size-desktop', numberDesktopSize + 'px');
        numberOne.style.setProperty('--number-font-size-mobile', numberMobileSize + 'px');
        numberOne.style.fontSize = 'var(--number-font-size-desktop)';
        ...
        const textDesktopSize = slide.textFontSize || 16;
        const textMobileSize = slide.textFontSizeMobile || slide.textFontSize || 14;
        textOne.style.setProperty('--text-font-size-desktop', textDesktopSize + 'px');
        textOne.style.setProperty('--text-font-size-mobile', textMobileSize + 'px');
        textOne.style.fontSize = 'var(--text-font-size-desktop)';
        ...
        // Apply font sizes with CSS custom properties for responsive design
        numberTwo.style.setProperty('--number-font-size-desktop', numberDesktopSize + 'px');
        numberTwo.style.setProperty('--number-font-size-mobile', numberMobileSize + 'px');
        numberTwo.style.fontSize = 'var(--number-font-size-desktop)';
        ...
        // Apply font sizes with CSS custom properties for responsive design
        textTwo.style.setProperty('--text-font-size-desktop', textDesktopSize + 'px');
        textTwo.style.setProperty('--text-font-size-mobile', textMobileSize + 'px');
        textTwo.style.fontSize = 'var(--text-font-size-desktop)';
    ...
    function createButton(buttonConfig) {
        ...
        const originalUrl = buttonConfig.url || '#';
        const finalUrl = wrapWithGamTracking(originalUrl, 'Button Click');
    ...
    // Analytics Tracking - Completely Anonymous
    // Global tracking function that can be called from anywhere
    ...
    function setupAnalytics() {
        ...
        function getCurrentSlideIndex() {
            ...
            const slideWidth = scrollContainer.querySelector('.content-box')?.offsetWidth || 340;
            return Math.round(scrollContainer.scrollLeft / slideWidth);
        }
        ...
        async function trackEvent(eventType, data) {
            try {
                ...
                // Silent fail - don't break carousel if analytics fails
                console.debug('Analytics error:', err);
            }
        }

        // Expose tracking function globally within this IIFE
        globalTrackEvent = trackEvent;

        // 1. Track carousel impression (once per pageview)
            ...
            const observer = new IntersectionObserver((entries) => {
                if (entries[0].isIntersecting && !impressionTracked) {
                    ...
                    trackEvent('impression', {
                        slide_index: 0,
                        total_slides: config.slides.length
                    });
                }
            }, { threshold: 0.5 });

            const carousel = document.getElementById('carouselRoot');
        ...
        function setupButtonTracking() {
            document.addEventListener('click', (e) => {
                const button = e.target.closest('.content-button');
                if (button) {
        ...
        // 3. Track navigation arrow clicks
        function setupNavigationTracking() {
            ...
            scrollButtonLeft?.addEventListener('click', () => {
                ...
                trackEvent('arrow_click', {
                    slide_index: currentSlide,
                    direction: 'left',
                    from_slide: currentSlide,
                    to_slide: Math.max(0, currentSlide - 1)
                });
            });
            ...
            scrollButtonRight?.addEventListener('click', () => {
                ...
                trackEvent('arrow_click', {
                    ...
                    direction: 'right',
                    from_slide: currentSlide,
                    to_slide: Math.min(totalSlides - 1, currentSlide + 1)
                });
            });
        }

        // 4. Track manual scroll/swipe
        function setupScrollTracking() {
            const scrollContainer = document.getElementById('scrollContainer');
            if (!scrollContainer) return;

            scrollContainer.addEventListener('scroll', () => {
                // Only track if it's a user-initiated scroll (not auto-scroll)
                if (!isAnimating) {
                    clearTimeout(scrollTimeout);

                    scrollTimeout = setTimeout(() => {
                        const endSlide = getCurrentSlideIndex();

                        // Only track if slide changed
                        if (startSlide !== endSlide) {
                            trackEvent('manual_scroll', {
                                slide_index: endSlide,
                                from_slide: startSlide,
                                to_slide: endSlide,
                                direction: endSlide > startSlide ? 'right' : 'left',
                                distance: Math.abs(endSlide - startSlide)
                            });
                        }
        ...
        function setupSwipeTracking() {
            ...
            let swipeStartSlide = 0;

            scrollContainer.addEventListener('touchstart', (e) => {
                touchStartX = e.touches[0].clientX;
                touchStartTime = Date.now();
                swipeStartSlide = getCurrentSlideIndex();
            }, { passive: true });
            ...
            scrollContainer.addEventListener('touchend', (e) => {
                ...
                // Only track significant swipes (>50px) that changed slides
                if (Math.abs(swipeDistance) > 50 && swipeStartSlide !== endSlide) {
                    trackEvent('swipe', {
                        slide_index: endSlide,
                        direction: swipeDistance > 0 ? 'left' : 'right',
                        from_slide: swipeStartSlide,
                        to_slide: endSlide,
                        distance_px: Math.abs(swipeDistance),
